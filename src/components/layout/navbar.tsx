...
export function Navbar() {
  ...
  const learningItems = [
    {
      label: t('common:navigation.labs'),
      href: ROUTES.COURSES.LIST,
      icon: <Code className='w-4 h-4' />,
      description: t('common:navigation.labsDesc'),
    },
    {
      label: t('common:navigation.paths'),
      href: '/paths',
      icon: <GraduationCap className='w-4 h-4' />,
      description: t('common:navigation.pathsDesc'),
      badge: 'Soon',
    },
    {
      ...
      description: t('common:navigation.challengesDesc'),
      badge: 'Soon',
    },
  ];
      ...
      <nav className='sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60'>
        <div className='container flex h-16 items-center justify-between gap-4 mx-auto'>
          ...
          <div className='hidden lg:flex items-center gap-6 flex-1 justify-center'>
            ...
            <NavDropdown
              label={t('common:navigation.learning')}
              items={learningItems}
            />
            <Link
              to={ROUTES.ABOUT}
            ...
            {isAuthenticated ? (
              ...
              <div className='hidden md:flex items-center gap-2'>
                <Button variant='ghost' size='sm' asChild>
                  <Link to={ROUTES.AUTH.LOGIN}>{t('common:auth.login')}</Link>
                </Button>
                ...
                <nav className='flex flex-col gap-4 mt-8'>
                  ...
                  {!isAuthenticated && (
                    <div className='flex flex-col gap-2 pt-4 border-t'>
                      <Button asChild>
                        <Link to={ROUTES.AUTH.LOGIN}>{t('auth:login')}</Link>
                      </Button>
